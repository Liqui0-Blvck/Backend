...
class RolePermissionMixin:
    def get_permissions(self):
        ...
        # Puedes agregar l贸gica granular aqu铆 por rol
...
class FruitLotViewSet(RolePermissionMixin, viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        qs = super().get_queryset()
        # Verificar si hay un filtro espec铆fico por estado_lote
        estado_lote = self.request.query_params.get('estado_lote', None)
        
        # Si se solicita espec铆ficamente un estado, aplicar ese filtro
        if estado_lote is not None:
            return qs.filter(estado_lote=estado_lote)
        
        # Si no hay filtro espec铆fico, ocultar los lotes agotados por defecto
    ...
    def update_maturation(self, request, uid=None):
        """Actualiza el estado de maduraci贸n de un lote de fruta"""
        ...
        if serializer.is_valid():
            ...
            # Verificar si el estado de maduraci贸n cambi贸
            if 'estado_maduracion' in request.data and lote_actualizado.estado_maduracion != estado_anterior:
                # Registrar el cambio en el historial de maduraci贸n
            ...
            return Response({
                'status': 'success',
                'message': 'Estado de maduraci贸n actualizado correctamente',
                'data': serializer.data
            }, status=status.HTTP_200_OK)
        
        return Response({
            'status': 'error',
            'message': 'Error al actualizar estado de maduraci贸n',
            'errors': serializer.errors
        }, status=status.HTTP_400_BAD_REQUEST)
    ...
    def crear_recepcion(self, request):
        ...
        if detalles_str and isinstance(detalles_str, str):
            try:
                ...
                return Response({'detalles': 'Formato JSON inv谩lido'}, status=status.HTTP_400_BAD_REQUEST)
        else:
        ...
        try:
            ...
            for i, detalle in enumerate(detalles):
                try:
                    ...
                    if detalle_serializer.is_valid():
                        ...
                        print(f"Errores de validaci贸n: {detalle_serializer.errors}")
            ...
            # Si hay errores en los detalles pero la recepci贸n se cre贸, reportarlos
            if errores_detalles:
                # Incluir los errores en la respuesta pero seguir con c贸digo 201 ya que la recepci贸n se cre贸
            ...
            # Si todo sali贸 bien, devolver respuesta completa
            respuesta = recepcion_serializer.data
            respuesta['detalles'] = detalles_creados
...
class SupplierViewSet(RolePermissionMixin, viewsets.ModelViewSet):
    ...
    def transactions(self, request, uid=None):
        """
        Obtiene todas las recepciones de mercanc铆a (GoodsReception) vinculadas a este proveedor
        """
        try:
...
class ReceptionDetailViewSet(RolePermissionMixin, viewsets.ModelViewSet):
    ...
    def bulk_create(self, request):
        ...
        for i, detalle in enumerate(data):
            ...
            # Eliminar el campo business si est谩 presente
...
class SupplierPaymentViewSet(RolePermissionMixin, viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        # Filtrar por recepci髇 si se proporciona el par醡etro
        recepcion_uid = self.request.query_params.get('recepcion', None)
        if recepcion_uid:
            qs = qs.filter(recepcion__uid=recepcion_uid)
        
        # Filtrar por proveedor si se proporciona el par醡etro
